...
public class PacketLoseFluid2 implements IPacket
{
    ...
    public void process(Supplier<NetworkEvent.Context> context)
    {
        context.get().enqueueWork(() ->
        {
            ...
            Map<BlockPos, FluidStack> map = Geothermal.openedPipes.getOrDefault(chunkPos, new HashMap<>());
            if(this.fluidStack.isEmpty())
                map.remove(this.pos);
            else
                map.put(this.pos, this.fluidStack);
            Geothermal.openedPipes.put(chunkPos, map);
        });
